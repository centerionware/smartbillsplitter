...
const SettingsComponent: React.FC<SettingsProps> = ({ settings, onUpdateSettings, onBack, subscriptionStatus, expirationDate, onLogout, requestConfirmation }) => {
  ...
  const isDirty = useMemo(() => {
    ...
    if (!initialDetails) return Object.values(formData).some(v => v);
    return (
  ...
  const handleExport = async () => {
    try {
      ...
      const settingsFromDb = await dbService.getSettings();

      // Data from localStorage
      const theme = localStorage.getItem('theme');
      const subStatusRaw = localStorage.getItem('smart-bill-splitter-subscription-status');

      const dataToExport: Record<string, any> = {
        'smart-bill-splitter-bills': bills,
        'smart-bill-splitter-settings': settingsFromDb,
      };

      if (theme) dataToExport['theme'] = theme;
      if (subStatusRaw) {
        // The raw value is a JSON string. Parse it so the export file contains a clean object.
        dataToExport['smart-bill-splitter-subscription-status'] = JSON.parse(subStatusRaw);
      }
  ...
  const handleImport = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const expectedKeys = ['smart-bill-splitter-bills', 'smart-bill-splitter-settings'];
        ...
        requestConfirmation(
          ...
          async () => {
            ...
            if (data['smart-bill-splitter-subscription-status']) {
              // The data from the file is a JS object, stringify it for localStorage.
              localStorage.setItem('smart-bill-splitter-subscription-status', JSON.stringify(data['smart-bill-splitter-subscription-status']));
            } else {
              localStorage.removeItem('smart-bill-splitter-subscription-status');
            }
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-white dark:bg-slate-800 p-8 rounded-lg shadow-lg">
        ...
        <div className="space-y-4">
          ...
          {subscriptionStatus === 'subscribed' && expirationDate ? (
              <div className="p-4 bg-teal-50 dark:bg-teal-900/40 border-l-4 border-teal-500 rounded-r-lg">
                  <p className="text-sm text-teal-800 dark:text-teal-200">
                      <span className="font-bold">Status:</span> Pro Tier
                  </p>
                  <p className="text-sm text-teal-800 dark:text-teal-200 mt-1">
                      <span className="font-bold">Expires on:</span> {expirationDate.toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric' })}
                  </p>
              </div>
          ) : (
              <>
