...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const filteredBills = useMemo(() => {
    ...
    // Apply summary filter only on active bills
    if (dashboardStatusFilter === 'active') {
        if (dashboardSummaryFilter === 'othersOweMe') {
            statusFilteredBills = statusFilteredBills.filter(bill => {
                const iOweOnThisBill = bill.participants.some(p => p.name.toLowerCase().trim() === myNameLower && !p.paid);
                const othersOweOnThisBill = bill.participants.some(p => p.name.toLowerCase().trim() !== myNameLower && !p.paid);
                return othersOweOnThisBill && !iOweOnThisBill;
            });
        } else if (dashboardSummaryFilter === 'iOwe') {
            statusFilteredBills = statusFilteredBills.filter(bill => 
                bill.participants.some(p => p.name.toLowerCase().trim() === myNameLower && !p.paid)
            );
        }
    }
  ...
  const filteredImportedBills = useMemo(() => {
    ...
    if (dashboardStatusFilter === 'active') {
        if (dashboardSummaryFilter === 'othersOweMe') {
            return []; // Can't owe me on an imported bill
        }
        if (dashboardSummaryFilter === 'iOwe') {
            return baseFiltered.filter(bill => !bill.localStatus.myPortionPaid);
        }
    }
    return baseFiltered;
  ...
  const getEmptyState = () => {
    ...
    if (dashboardSummaryFilter === 'othersOweMe' && filteredBills.length === 0) {
        return {
            title: "All Caught Up!",
            message: "No one currently owes you money on active bills.",
        };
    }
     if (dashboardSummaryFilter === 'iOwe' && filteredBills.length === 0 && filteredImportedBills.length === 0) {
        return {
            title: "You're All Paid Up!",
            message: "You don't currently owe money on any active bills.",
        };
    }
